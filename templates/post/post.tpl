<div class="postContainer active styleColorBorder" rel="{if $item.type=='poll'}poll{else}post{/if}_{$item.id}">
	{include file='post/postStamp.tpl'}
	{if $item.type=='poll'}
		{include file='post/poll.tpl'}
	{else}
		{if $item.caption}<h2><a href='p{$item.id}' class="styleColor">{$item.caption}</a></h2>{/if}
