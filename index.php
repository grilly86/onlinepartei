	...
	class Workspace
	{
		...
		function process()
		{
				...
				switch($this->task)
				{
							...
							if (isset($_POST["parentID"]) && isset($_POST["message"]))
							{
								...
								if (isset($_POST["id"]) && $_POST["id"]>0)
								{
									...
									if ($id == $parentID)
									{
										...
										if ($type == 'poll')
										{
											$sql = "UPDATE poll SET `text`='" . $message . "',messageHtml='" . $messageHtml . "' " . $sqlSet . " WHERE id=" . $id . " AND (userID=" . $userID . " OR userID=0)";
										}
										else
										{
					...
					case "post":
						...
						if ((isset($id) && $postID = (int)$id) )
						{
							...
							{
								$obj = $this->getPostList("poll.id=" . $id,'timestamp DESC ', '','poll');
							}
							else
							{
								$obj = $this->getPostList("post.id=" . $id,'timestamp DESC ', '','post');
			...
			else
			{
				...
				$this->smarty->assign("TPL_POSTS", $this->smarty->fetch("error/500.tpl"));
				$this->smarty->assign("loggedIn", (int)$loggedIn);
				$this->smarty->assign("user", $this->user);
				$this->smarty->assign("pageTitle", $pageTitle);
				if (isset($pageDescription)) $this->smarty->assign("pageDescription", $pageDescription);
